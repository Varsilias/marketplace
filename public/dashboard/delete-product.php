...
$message = [];
function deleteProduct(int $productId, int $userId) {
    $query = new ProductRequest();
    $message = $query->deleteProduct($productId, $userId);
    return $message;
}
...
if (isset($_GET['id'])) {
    ...
    $productId = $_GET['id'];
    $status = deleteProduct($productId, $_SESSION['id']);
    if($status == 'One record affected') {
        $message['success'] = 'Successfully deleted Product';
    } else {
        $message['error'] = 'An error occured while deleting specified Product';
    }
...
<head>
    ...
    <title>Product deleted successfully | Market Place</title>
